...
- (void)layoutStatus:(NSView*)container
{
    ...
    if( [NSStringFromClass([container class]) isEqualToString:@"IDEComparisonEditorAutoLayoutView"] ){
        ...
        if( [container subviews].count > 0 ){
            [[container subviews][0] setFrame:NSMakeRect(0, STATUS_LINE_HEIGHT, parent.size.width, parent.size.height-STATUS_LINE_HEIGHT)];
        }
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    if( [keyPath isEqualToString:@"document"] ){
        ...
        {
            [object removeObserver:self forKeyPath:@"document"];
            [_status setString:@""];
            return;
        }
        NSString *path = [[[object document] fileURL] path];
        if (path != nil) {
