...
- (NSString *)string
{
	return ((NSTextView*)_view).string;
}
...
{ // C-y
  ...
  NSRect currentInsertionRect = [[(NSTextView*)_view layoutManager] boundingRectForGlyphRange:NSMakeRange(index,0) inTextContainer:[(NSTextView*)_view textContainer]];
  ...
  if (relativeInsertionPoint.y > visibleRect.size.height) {
    [_view moveUp:self];
    NSPoint newPoint = [[(NSTextView*)_view layoutManager] boundingRectForGlyphRange:[(NSTextView*)_view selectedRange] inTextContainer:[(NSTextView*)_view textContainer]].origin;
    index = [[(NSTextView*)_view layoutManager] glyphIndexForPoint:newPoint inTextContainer:[(NSTextView*)_view textContainer]];
  }
  return index;
}
...
- (NSUInteger)halfPageScrollHelper:(NSUInteger)index count:(NSInteger)count
{
    ...
    NSTextContainer *container = [(NSTextView*)_view textContainer];
    ...
    NSRect currentInsertionRect = [[(NSTextView*)_view layoutManager] boundingRectForGlyphRange:NSMakeRange(index,0) inTextContainer:container];
    NSPoint relativeInsertionPoint = XVimSubPoint(currentInsertionRect.origin, visibleRect.origin);
    ...
    NSUInteger cursorIndexAfterScroll= [[(NSTextView*)_view layoutManager] glyphIndexForPoint:cursorAfterScroll inTextContainer:container fractionOfDistanceThroughGlyph:NULL];
