...
+ (void)initKeyCodeToSelectorString
{
	if (!s_keyCodeToSelectorString)
	{
		...
		{
			keyCodeToSelectorString[@(i)] = key;
		};
		
		for (int i = 0; i < 128; ++i)
		{
			NSString *string = @(keynames[i]);
			map(string, i);
		}
...
- (instancetype)initWithKeyCode:(unichar)keyCode
...
static NSString* toSelectorString(unichar charcode, NSUInteger modifierFlags)
{
	...
	NSString *keyname = s_keyCodeToSelectorString[[NSNumber numberWithUnsignedShort:charcode]];
	if (keyname) { 
...
- (NSString*) toString 
{
	...
	if (charcode <= 127)
	{
		NSString *keyname = @(readable_keynames[charcode]);
		if (keyname) { 
			[keyStr appendString:keyname];
		}
	}
...
+ (XVimKeyStroke *)fromString:(NSString *)string from:(NSUInteger*)index
{
	...
	number = number ? number : s_stringToKeyCode[keyString];
	number = number ? number : s_stringToKeyCode[[keyString uppercaseString]];
